local MovementConfig = {
	DebugMovement = false,

	SlideKeyCodes = {
		Enum.KeyCode.LeftControl,
		Enum.KeyCode.C,
		Enum.KeyCode.ButtonX,
	},

	SlideDuration = 0.45,
	SlideCooldown = 0.9,
	SlideBoostSpeed = 36,
	MinSlideSpeed = 10,
	SlideFriction = 0.92,
	MaxHorizontalSpeed = 60,
	SlideSteerStrength = 0.42,
	SlideSteerResponsiveness = 0.16,
	SlideMaxTurnRate = 7.5,

	GroundCheckDistance = 5,
	MaxSlopeAngle = 50,

	MaxJumps = 2,
	AirJumpPower = 47,
	AirJumpCooldown = 1.0,
	CanAirJumpAfterGroundLeaveDelay = 0.12,
	LandingResetDelay = 0.05,

	PreserveHorizontalMomentum = true,
	MaxMomentumChainSpeed = 110,
	MomentumChainMinSpeed = 3,
	MomentumChainAirDrag = 1.0,
	MomentumChainAllowAirControl = true,
	MomentumChainAirControlStrength = 0.018,
	MomentumChainAirTurnResponsiveness = 0.012,

	AirControlEnabled = true,
	AirControlStrength = 0.04,
	AirDrag = 0.999,
	MinAirSpeedToPreserve = 6,
	MaxAirHorizontalSpeed = 60,
	AirTurnResponsiveness = 0.025,

	FaceMovementDirectionEnabled = true,
	SlideFaceTurnSpeed = 14,
	AirFaceTurnSpeed = 10,

	LowGravityEnabled = true,
	AirGravityScale = 0.70,
	LowGravityMinYVelocity = -140,
	SlideJumpGravityScale = 0.68,
	SlideJumpLowGravityDuration = 0.35,
	MomentumChainGravityScale = 0.70,

	SlopeSlideEnabled = true,
	SlopeAcceleration = 22,
	UphillDeceleration = 18,
	FlatSlideFriction = 0.9965,
	DownhillSpeedGainMultiplier = 0.65,
	UphillSlowdownMultiplier = 0.65,
	MinSlideEndSpeed = 3,
	MaxSlideSlopeSpeed = 100,
	SlideGroundGraceTime = 0.16,
	SlideSurfaceTransitionGrace = 0.2,
	SlideSurfaceTransitionFriction = 0.9985,
	MinSlopeAutoSlideAngle = 8,
	SlopeAutoSlideStartSpeed = 4,
	SlideEdgeLaunchEnabled = true,
	SlideEdgeLaunchSpeedMultiplier = 1.0,
	SlideEdgeLaunchUpVelocity = 3,
	SlidePreserveYVelocity = true,
	SlideJumpEnabled = true,
	SlideJumpHorizontalMultiplier = 1.08,
	SlideJumpExtraUpVelocity = 3,
	SlideJumpMinCarrySpeed = 3,
	SlideJumpMaxCarrySpeed = 110,
	SlideJumpCountsAsJump = true,

	SlideHoldEnabled = true,
	SlideHardMaxDurationEnabled = false,
	MaxSlideHoldDuration = 1.2,

	LandingMomentumCarry = true,
	LandingFrictionDuration = 0.12,
	LandingFriction = 0.9,
	SlideLandingResumeEnabled = true,
	SlideLandingResumeGraceTime = 0.25,
	SlideLandingMinCarrySpeed = 10,
	SlideLandingBypassCooldown = true,
	SlideLandingNoFreshBoost = true,
	SlideLandingFriction = 0.995,

	-- Compatibility tuning for existing Level movement upgrades.
	SlideBaseSpeedMultiplier = 1.55,
	SlideSpeedPerLevel = 0.18,
	SlideDurationPerLevel = 0.03,
	MaxSlideDurationBonus = 0.18,
	SlideCooldownReductionPerLevel = 0.10,
	MinSlideCooldown = 0.45,
	SlideLookFallbackMinSpeed = 2.5,
	SlideCameraDrop = 1.15,

	SlideActiveAttribute = "MovementSlideActive",
	SlideAnimationId = "rbxassetid://78771843103599",
	SlideAnimationFadeTime = 0.1,
	SlideAnimationSpeed = 1,
}

return table.freeze(MovementConfig)
