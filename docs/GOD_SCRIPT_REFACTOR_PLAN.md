...
| `Four Peaks/StarterPlayer/StarterPlayerScripts/BlacksmithUI.lua` | 1121 | 38 / 0 | 6 | `OpenBlacksmithUI`, `BlacksmithSync`, `BlacksmithAction` | event-driven UI, 15 connections; scene lifecycle delegated | 0 / 0 |
...
| `Four Peaks/StarterPlayer/StarterPlayerScripts/BlacksmithEntryBuilder.lua` | 124 | 1 / 7 | 0 | none | no runtime loop or connection | 0 / 0 |
| `Four Peaks/StarterPlayer/StarterPlayerScripts/BlacksmithSceneController.lua` | 338 | 9 / 13 | 0 | none | existing scene `BindToRenderStep` plus 3 connections moved from `BlacksmithUI` | 0 / 0 |
...
- 6D ukończony.
- 6E ukończony.
- Etap 6 ukończony po audycie repo/Studio 2026-07-07.
...
- 6E dodał `Four Peaks/ServerScriptService/ModuleScript/GuildUpdateBroadcaster.lua`.
- `GuildUpdateBroadcaster` odpowiada za odzyskanie/utworzenie istniejącego `RemoteEvents.GuildUpdated`, payload `{ GuildId, Reason, ServerNowUnix }` i wysyłkę do członków/extra graczy.
- `GuildService` po 6E nadal odpowiada za DataStore load/save, directory, membership, join/invite flow, treasury donate/spend, upgrades, task progress, teleport i publiczne API.
- Graf 6E: `GuildService -> GuildRecordState`, `GuildService -> GuildUpdateBroadcaster`, `GuildService -> GuildConfig`, `GuildService -> PlayerData`, `GuildService -> CurrencyService`; `GuildRecordState -> GuildConfig`; `GuildUpdateBroadcaster` nie wymaga modułów. Nie powstał cykl require.
...
- Runtime 6E: nie dodano pętli, schedulerów, `_G`, connection ani fallbacków; przeniesiono istniejący `GuildUpdated` remote factory/broadcast do modułu bez zmiany nazwy remote ani payloadu.
...
- Walidacja 6E: repo/Studio parity aktywnego `Four Peaks` potwierdzona bajtowym length/hash: `GuildService` `36581/45412315`, `GuildRecordState` `9763/53470018`, `GuildUpdateBroadcaster` `1721/304481289`.
- Audyt zamknięcia 6: `Guild` parity potwierdzone dla `GuildPlace` `32813/312367508`, `GuildPlaceRemotes` `1923/248263159`, `GuildPlaceLocations` `11559/420695235`; `Four Peaks` parity potwierdzone dla `GuildService` `36581/45412315`, `GuildRecordState` `9763/53470018`, `GuildUpdateBroadcaster` `1721/304481289`.
- Audyt zamknięcia 6: repo grep dla aktywnych modułów Guild nie wykazał nowych `_G`, nowych pętli frame ani nowych connection poza istniejącymi player/remote/prompt bindingami; `GuildPlace` ma `1063` linii, `GuildService` ma `1159` linii.
...
- Play test 6E w aktywnym `Four Peaks`: `GuildService.GetState` zwrócił read-only state z `Success=true`, `Config` i `PlayerResources`; syntetyczny `GuildUpdateBroadcaster.Broadcast` potwierdził, że `RemoteEvents.GuildUpdated` pozostaje `RemoteEvent`.
- Nieweryfikowane w 6E: create/join guild, treasury deposit/spend, upgrade, task progress i teleport do/z lobby nie były wykonywane, ponieważ checkpoint nie zmieniał DataStore write flow, treasury math ani teleport handlerów.
...
- 7F ukończony.
- 7G ukończony.
- Etap 7 ukończony po audycie: aktywny `BlacksmithUI` spadł poniżej progu 1200 linii, a `InventoryController` pozostaje większy, ale po 7A/7C/7D/7G jego pozostała odpowiedzialność to layout/render/action owner inventory. Dalsze dzielenie kart/detali inventory wymagałoby osobnego, większego UI renderer refactoru, więc nie było kontynuowane w tym etapie bez nowego kontraktu.
...
- 7C dodał `Four Peaks/StarterPlayer/StarterPlayerScripts/InventoryEntryBuilder.lua`.
- `InventoryEntryBuilder` odpowiada za dotychczasową transformację snapshotu na listy weapon/spell/material/codex entries, material usage metadata i definicje materiałów.
- `InventoryController` po 7C nadal odpowiada za UI state, layout, filtry, sortowanie, detale, akcje inventory, snapshot loading, remotes i input bindings.
- Graf 7C: `InventoryController -> InventoryEntryBuilder`; `InventoryEntryBuilder` nie wymaga modułów i używa przekazanych zależności `WeaponConfigs`, `SpellDefs`, `CraftingConfig`, `MaterialDefinitions`.
- Runtime 7C: nie dodano pętli, schedulerów, remotes, `_G`, connection ani fallbacków; przeniesiono wyłącznie czystą transformację danych snapshotu.
- Walidacja 7C: repo/Studio parity aktywnego `Four Peaks` potwierdzona bajtowym length/hash: `InventoryController` `76784/270817220`, `InventoryIconResolver` `3898/377823037`, `InventoryEntryBuilder` `7480/197435266`.
- Play test 7C w aktywnym `Four Peaks`: inventory otwarte przez istniejące atrybuty `ScreenButtonsAction=open` i `ScreenButtonsNonce`; `InventoryGui.Enabled=true`, `Modal=true`, `InventoryEntryBuilder` obecny w `PlayerScripts` jako `ModuleScript`, `require` zwrócił table, render zawierał `10` obrazów z assetami, `22` przyciski i `81` labeli; syntetyczny builder zwrócił counts `weapons=1`, `spells=0`, `materials=0`, `codex=1`.
- Nieweryfikowane w 7C: ręczne kliknięcie każdej zakładki, equip/favorite/sell i wszystkie sort/filter kombinacje nie były wykonywane, ponieważ checkpoint zmieniał tylko transformację danych snapshotu.
- Rollback 7C: przywrócić inline `materialUses`, `getWeaponDef`, `buildWeaponEntries`, `buildSpellEntries`, `getMaterialDefinition`, `addMaterialEntries`, `buildMaterialEntries` i `buildCodexEntries` w `InventoryController.lua`, usunąć `InventoryEntryBuilder.lua` z repo i live Studio oraz cofnąć wpisy planu/changeloga.
- 7D dodał `Four Peaks/StarterPlayer/StarterPlayerScripts/InventoryFilterSorter.lua`.
- `InventoryFilterSorter` odpowiada za dotychczasowe filtrowanie search/filter tabs oraz sortowanie entries dla weapon/spell/material/codex tabs.
- `InventoryController` po 7D nadal odpowiada za UI state, layout, detale, akcje inventory, snapshot loading, remotes, input bindings i wywołanie obecnego sort/filter flow.
- Graf 7D: `InventoryController -> InventoryFilterSorter`; `InventoryFilterSorter` nie wymaga modułów i używa przekazanych zależności `RarityOrder`, `TextContains` i `GetSpellDamage`.
- Runtime 7D: nie dodano pętli, schedulerów, remotes, `_G`, connection ani fallbacków; przeniesiono wyłącznie czystą selekcję listy i `table.sort`.
- Walidacja 7D: repo/Studio parity aktywnego `Four Peaks` potwierdzona bajtowym length/hash: `InventoryController` `74327/316184324`, `InventoryFilterSorter` `3760/478229585`.
- Play test 7D w aktywnym `Four Peaks`: czysta sesja Play otworzyła inventory przez istniejące atrybuty `InventoryGui.ScreenButtonsAction=open` i `ScreenButtonsNonce`; `InventoryFilterSorter` był obecny w `PlayerScripts` jako `ModuleScript`, `require` zwrócił table, render po realnym snapshot load miał `10` obrazów z assetami, `22` przyciski, `81` labeli i `1` TextBox. Search TextBox przez realny event `Text` ograniczył widok do `1` obrazu z assetem, `10` przycisków i `32` labeli, a wyczyszczenie search przywróciło `10`/`22`/`81`; Output bez błędów UI/filter sorter.
...
- Audyt zamknięcia etapu 7: `InventoryController` ma jawny graf `InventoryController -> InventoryIconResolver/InventoryEntryBuilder/InventoryFilterSorter/InventoryCharacterPreview`; `BlacksmithUI` ma jawny graf `BlacksmithUI -> BlacksmithIconResolver/BlacksmithEntryBuilder/BlacksmithSceneController`. Nie dodano gameplayowych `_G`, remotes, persistent data, teleport data, fallbacków ani per-object Heartbeat. Testy Play dla 7A/7C/7D/7G pokryły realne otwarcie inventory, snapshot render, search filtering, preview clone i close toggle; testy Play dla 7B/7E/7F pokryły realny remote open blacksmith, render, entry builder i lifecycle sceny open/close.
...
- 7E dodał `Four Peaks/StarterPlayer/StarterPlayerScripts/BlacksmithEntryBuilder.lua`.
- `BlacksmithEntryBuilder` odpowiada za dotychczasowe budowanie craft entries per category, wybór aktywnej kategorii/recipe oraz teksty display name/type/stat lines dla blacksmith cards/details.
- `BlacksmithUI` po 7E nadal odpowiada za UI layout, kamerę, ukrywanie promptów/postaci, tooltipy, render kart/detali, crafting request, remotes i input bindings.
- Graf 7E: `BlacksmithUI -> BlacksmithEntryBuilder`; `BlacksmithEntryBuilder` nie wymaga modułów i używa przekazanych zależności `WeaponConfigs` oraz `ClampInt`.
- Runtime 7E: nie dodano pętli, schedulerów, remotes, `_G`, connection ani fallbacków; przeniesiono wyłącznie czyste helpery danych listy craftingu.
- Walidacja 7E: repo/Studio parity aktywnego `Four Peaks` potwierdzona bajtowym length/hash: `BlacksmithUI` `42036/419752749`, `BlacksmithEntryBuilder` `3580/312108726`.
- Play test 7E w aktywnym `Four Peaks`: `OpenBlacksmithUI:FireClient(player)` otworzył istniejące UI; `BlacksmithEntryBuilder` był obecny w `PlayerScripts` jako `ModuleScript`, `require` zwrócił table, render zawierał `44` obrazy z assetami, `13` przycisków (`12` widocznych) i `46` labeli; syntetyczny smoke potwierdził category grouping `Sword=1`, `Bow=1`, fallback selected category `Sword`, selected recipe `r1` i stat line `ATK 12`; Output bez błędów `BlacksmithUI`/`BlacksmithEntryBuilder`.
- Nieweryfikowane w 7E: craft/upgrade request, material tooltip matrix, ręczne kliknięcie każdej kategorii i pełne zamknięcie z kamerą nie były wykonywane, ponieważ checkpoint zmieniał tylko helpery danych listy craftingu.
- Rollback 7E: przywrócić inline `buildEntriesByCategory`, `getSelectedCategoryEntries`, `getSelectedEntry`, `getWeaponDisplayName`, `getWeaponTypeLabel` i `buildStatLines` w `BlacksmithUI.lua`, usunąć `BlacksmithEntryBuilder.lua` z repo i live Studio oraz cofnąć wpisy planu/changeloga.
- 7F dodał `Four Peaks/StarterPlayer/StarterPlayerScripts/BlacksmithSceneController.lua`.
- `BlacksmithSceneController` odpowiada za dotychczasowe ukrywanie/przywracanie lobby UI, kamery kowala, lokalnej postaci, stanu movementu oraz promptów kowala.
- `BlacksmithUI` po 7F nadal odpowiada za UI layout, tooltipy, render kart/detali, crafting request, remotes, input bindings i kolejność open/close flow.
- Graf 7F: `BlacksmithUI -> BlacksmithSceneController`; `BlacksmithSceneController` nie wymaga modułów i używa jawnie przekazanych zależności `Player`, `PlayerGui`, `Gui`, `CameraPoint`, `Workspace`, `RunService` oraz stałych kamery/lobby.
- Runtime 7F: nie dodano nowych pętli, remotes, `_G`, fallbacków ani schedulerów; istniejący `BindToRenderStep` kamery i 3 scenowe connection zostały przeniesione z `BlacksmithUI` do modułu.
- Walidacja 7F: repo/Studio parity aktywnego `Four Peaks` potwierdzona bajtowym length/hash: `BlacksmithUI` `35187/419464323`, `BlacksmithSceneController` `8781/892045071`.
- Play test 7F w aktywnym `Four Peaks`: `OpenBlacksmithUI:FireClient(player)` otworzył istniejące UI; `BlacksmithSceneController` był obecny w `PlayerScripts` jako `ModuleScript`, `require` zwrócił table, render zawierał `44` obrazy z assetami, `13` przycisków (`12` widocznych) i `46` labeli. Podczas open prompt kowala był ukryty `1/1`, `16` części lokalnej postaci miało `LocalTransparencyModifier >= 1`, FOV kamery wynosił `45`; po zamknięciu przez istniejący `BlacksmithCloseRequested` GUI było disabled, prompt wrócił `0/1` disabled, ukryte części postaci spadły do `0`, `ScreenGuiButtons.Enabled=true`, FOV wrócił do `70`; Output bez błędów `BlacksmithUI`/`BlacksmithSceneController`.
- Nieweryfikowane w 7F: ręczne kliknięcie BackButton i fizyczne przejście przez ProximityPrompt nie były wykonywane, ponieważ test użył istniejącego remote oraz istniejącego `BlacksmithCloseRequested`; craft/upgrade request i tooltip matrix pozostają poza zakresem scene extraction.
- Rollback 7F: przywrócić inline funkcje lifecycle sceny w `BlacksmithUI.lua` (`hideLobbyUi`, kamera, character visibility, movement snapshot/restore, prompt hide/restore, `isPromptInsideBlacksmith`), usunąć `BlacksmithSceneController.lua` z repo i live Studio oraz cofnąć wpisy planu/changeloga.
