...
local Torso, Hum = Character:FindFirstChild("Torso") or Character:FindFirstChild("UpperTorso"), Character:FindFirstChildOfClass("Humanoid")
...
if not IsTeamMate(Creator, game:GetService("Players"):GetPlayerFromCharacter(Character)) then
	...
	if game:GetService("Players"):GetPlayerFromCharacter(Character) then
		...
		SnatchedSoul.Color = Color3.fromRGB(255, 85, 0)
		...
		Touch = SnatchedSoul.Touched:Connect(function(hit)
			...
			if not Hum or game:GetService("Players"):GetPlayerFromCharacter(Hum.Parent) ~= Creator then
				return
			end
			if SoulCount then
