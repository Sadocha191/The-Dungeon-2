...
local function formatReward(reward: any): string
	local textParts = {}
	for _, rewardPart in ipairs(buildRewardParts(reward)) do
		textParts[#textParts + 1] = rewardPart.text
	end
...
local function refreshUI(preserveScroll: boolean?)
	local scrollState = preserveScroll and captureScrollState() or nil
