...
local normalizeElementName = BlacksmithIconResolver.NormalizeElementName
...
local function requestCraftAction(entry)
	...
	showPopup({
		title = "Confirm Forge",
		body = string.format("Forge %s for %d silver?", getWeaponDisplayName(WeaponConfigs.Get(entry.weaponId), entry), clampInt(entry.craftSilverCost, 0)),
		primaryText = "Confirm",
