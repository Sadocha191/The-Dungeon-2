...
local SpellDefinitions = require(moduleFolder:WaitForChild("SpellDefinitions"))
...
function PickupToastService.PushSilver(player, amount, note)
...
function PickupToastService.PushWeaponPoints(player, amount, note)
	amount = math.floor(tonumber(amount) or 0)
	if amount <= 0 then
		return
	end
	PickupToastService.Push(player, {
		variant = "weaponPoints",
		label = "Weapon Points",
		amount = amount,
		note = note,
	})
...
function PickupToastService.PushWeapon(player, weaponId, note, amount, rarityOverride)
	local def = WeaponConfigs.Get(weaponId)
	local label = (def and def.name) or tostring(weaponId or "")
	if label == "" then
		return
	end
	...
	PickupToastService.Push(player, {
		...
		note = note,
		rarity = rarityOverride or (def and def.rarity) or nil,
	})
end

function PickupToastService.PushSpell(player, spellIdOrName, note, amount)
	local label = ""
	local rarity = nil
	local product = SpellDefinitions.GetProduct(spellIdOrName)
	local spell = SpellDefinitions.GetSpell(spellIdOrName)

	if product then
		label = product.displayName or product.name or tostring(spellIdOrName or "")
		rarity = product.cardQuality or product.rarity
	elseif spell then
		label = spell.name or tostring(spellIdOrName or "")
	else
		label = tostring(spellIdOrName or "")
	end
